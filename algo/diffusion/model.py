"""Transformer-based conditional diffusion model for PushT actions."""
...
class DiffusionTransformerModel(nn.Module):
    """Predict diffusion noise on action conditioned on multimodal observations."""
    ...
    def forward(self, noisy_action: torch.Tensor, timestep: torch.Tensor, obs: dict[str, torch.Tensor]) -> torch.Tensor:
        obs_tokens, _ = self.obs_encoder(obs)
        return self.head(noisy_action, timestep, obs_tokens)
